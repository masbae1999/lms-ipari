...
class certificate {
    ...
    public static function generate_code() {
        ...
        $lastcert = $DB->get_record_sql("SELECT MAX(id) as lastid FROM {customcert_issues}");
        $nextnumber = $lastcert->lastid + 1; // Nomor sertifikat berikutnya

        // Buat format nomor sertifikat sesuai aturan
        $year = date("Y");
        $code = "A.$nextnumber/D.M/P.V/$year";
